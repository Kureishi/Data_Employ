...
> **Note:** In the sidebar's **Database** section, the button that connects to the database is labeled **Link** (rather than "Connect") so it fits the narrow left panel without text clipping. Its behavior is identical to a "Connect" button — enter a file path or `sqlite:///...` URL and click it to establish the connection.
